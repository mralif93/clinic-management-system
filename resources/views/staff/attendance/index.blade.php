...
@section('content')
    <div class="w-full space-y-6">
        <!-- Page Header -->
        <div class="bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600 rounded-2xl  p-6 text-white shadow-lg relative overflow-hidden">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    <div class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                        <i class='hgi-stroke hgi-clock-02 text-3xl'></i>
                    </div>
                    <div>
            ...
            @if($todayAttendance)
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 text-center">
                        ...
                        @endphp
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $sConfig['bg'] }} {{ $sConfig['text'] }}">
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                <div class="flex items-center justify-between">
                    <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                        <i class='hgi-stroke hgi-list-view text-blue-500'></i>
                        Attendance History
                    </h3>
                            ...
                            <tr class="hover:bg-blue-50/30 transition-colors duration-150">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            <span
                                                class="text-white text-sm font-bold">{{ $attendance->date->format('d') }}</span>
                                        </div>
                                        <div>
                                            <div class="text-sm font-semibold text-gray-900">
                                                {{ $attendance->date->format('l') }}</div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1 text-sm text-green-700 font-medium">
                                        <i class='hgi-stroke hgi-login-02'></i>
                                ...
                                <td class="px-6 py-4">
                                    @if($attendance->total_hours)
                                        <span
                                            class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg bg-blue-100 text-blue-700 text-sm font-bold">
                                    ...
                                    <div class="flex justify-end">
                                        ...
                                        @if($pendingCorrection)
                                            <span
                                                ...
                                                <i class='hgi-stroke hgi-clock-02'></i> Pending
                                            </span>
                                        @else
                                            <button
                                                onclick="openCorrectionModal('{{ $attendance->id }}', '{{ $attendance->date->format('Y-m-d') }}', '{{ $attendance->clock_in_time->format('H:i') }}', '{{ $attendance->clock_out_time ? $attendance->clock_out_time->format('H:i') : '' }}')"
        ...
        <script>
            function openCorrectionModal(id, date, clockIn, clockOut) {
                ...
                `,
                    ...
                    confirmButtonText: '<i class="hgi-stroke hgi-sent mr-1"></i> Submit Request',

...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Clinic Schedule</h1>
            <p class="text-gray-600 mt-1">View and manage clinic appointments</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" action="{{ route('staff.schedule.index') }}" class="flex items-center gap-4">
            ...
            <a href="{{ route('staff.schedule.index') }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400 transition">
                Today
            </a>
        </form>
    </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
                @for($date = $weekStart->copy(); $date <= $weekEnd; $date->addDay())
                    ...
                    @endphp
                    <div class="border border-gray-200 rounded-lg p-3 {{ $isToday ? 'bg-yellow-50 border-yellow-300' : ($isSelected ? 'bg-blue-50 border-blue-300' : '') }}">
                        ...
                        <div class="space-y-1">
                            @foreach($dayAppointments->take(3) as $appointment)
                                <div class="text-xs bg-yellow-100 text-yellow-800 rounded px-1 py-0.5 truncate" title="{{ $appointment->patient->full_name }}">

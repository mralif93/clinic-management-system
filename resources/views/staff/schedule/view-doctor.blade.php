        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($scheduleData as $dayIndex => $schedule)
                ...
                <div
                    ...
                    <!-- Card Header -->
                    <div
                        class="{{ $isActive ? 'bg-gradient-to-r from-yellow-500 to-yellow-600' : 'bg-gray-500' }} px-4 py-3 flex items-center justify-between">
                        <h3 class="text-lg font-bold text-white">{{ $dayName }}</h3>
                        <span
                            class="px-2 py-1 text-xs font-semibold rounded-full {{ $isActive ? 'bg-white text-yellow-800' : 'bg-gray-200 text-gray-800' }}">

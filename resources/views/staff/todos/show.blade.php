    ...
    <div class="space-y-6">
            ...
            <div class="relative p-6 md:p-8">
                <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="px-3 py-1.5 {{ $currentPriority['bg'] }} {{ $currentPriority['text'] }} rounded-full text-sm font-semibold flex items-center gap-1">
                            <i class='hgi-stroke {{ $currentPriority['icon'] }}'></i>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 {{ $currentPriority['bg'] }} rounded-lg flex items-center justify-center">
                        <i class='hgi-stroke {{ $currentPriority['icon'] }} {{ $currentPriority['text'] }} text-xl'></i>
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 {{ $todo->due_date && $todo->isOverdue() ? 'bg-red-100' : 'bg-gradient-to-br from-teal-500 to-cyan-600' }} rounded-lg flex items-center justify-center">
                        <i
                            class='hgi-stroke hgi-calendar-03 {{ $todo->due_date && $todo->isOverdue() ? 'text-red-600' : 'text-white' }} text-xl'></i>
                    </div>
                    <div>
                        @if($todo->due_date)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                            <i class='hgi-stroke hgi-task-01 text-teal-500'></i>
                            Task Information
                        </h3>
                    </div>
                    <div class="p-6">
                        ...
                        @else
                            <div class="bg-gray-50 p-4 rounded-xl text-center">
                                <i class='hgi-stroke hgi-information-circle text-gray-400 text-2xl'></i>
                ...
                @if($todo->status !== 'completed')
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                            <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                                <i class='hgi-stroke hgi-refresh text-teal-500'></i>
                                Update Status
                            </h3>
                        </div>
                        <div class="p-6">
                            <form action="{{ route('staff.todos.update-status', $todo->id) }}" method="POST"
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="relative">
                                        <i
                                            class='hgi-stroke hgi-loading-03 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400'></i>
                                        <select name="status" required
                                            ...
                                            @foreach(\App\Models\Todo::getStatuses() as $key => $label)
                                                <option value="{{ $key }}" {{ $todo->status == $key ? 'selected' : '' }}>{{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                            <i class='hgi-stroke hgi-information-circle text-teal-500'></i>
                            Details
                        </h3>
                    </div>
                    <div class="p-4 space-y-4">
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $todo->created_at->format('M d, Y h:i A') }}
                                </p>
                            </div>
                        </div>
                        @if($todo->updated_at && $todo->updated_at != $todo->created_at)
                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                                <i class='hgi-stroke hgi-pencil-edit-01 text-gray-400'></i>
                                <div>
                                    <p class="text-xs text-gray-500">Last Updated</p>
                                    <p class="text-sm font-medium text-gray-900">{{ $todo->updated_at->format('M d, Y h:i A') }}
                                    </p>
                                </div>
                            </div>
                        @endif
                ...
                @if($todo->status === 'completed')
                    <div
                        class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl  p-6 text-white shadow-lg relative overflow-hidden">
                        <!-- Decorative background elements -->
                        <div class="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>
                        <div class="absolute bottom-0 left-0 -mb-8 -ml-8 w-40 h-40 bg-white/10 rounded-full blur-3xl"></div>
                        <div class="flex items-center gap-4">
                            <div
                                class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                                <i class='hgi-stroke hgi-checkmark-circle-02 text-3xl'></i>
                            </div>
                            <div>
                ...
                @elseif($todo->due_date && $todo->isOverdue())
                    <div
                        class="bg-gradient-to-br from-red-500 to-rose-600 rounded-xl  p-6 text-white shadow-lg relative overflow-hidden">
                        <!-- Decorative background elements -->
                        <div class="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>
                        <div class="absolute bottom-0 left-0 -mb-8 -ml-8 w-40 h-40 bg-white/10 rounded-full blur-3xl"></div>
                        <div class="flex items-center gap-4">
                            <div
                                class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                                <i class='hgi-stroke hgi-alert-circle text-3xl'></i>
                            </div>
                            <div>

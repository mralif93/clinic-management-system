...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" action="{{ route('staff.appointments.index') }}" class="flex flex-wrap gap-4">
            ...
            <div>
                <select name="status" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
            ...
            <div>
                <select name="doctor_id" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
            ...
            @if(request()->hasAny(['search', 'status', 'doctor_id', 'date']))
                <a href="{{ route('staff.appointments.index') }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400 transition">
                    Clear
                </a>
            @endif
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($appointments as $appointment)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $appointment->doctor->full_name ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('staff.appointments.show', $appointment->id) }}" class="text-yellow-600 hover:text-yellow-900" title="View">
                                        <i class='bx bx-show text-xl'></i>
                                    </a>
                                    <a href="{{ route('staff.appointments.edit', $appointment->id) }}" class="text-blue-600 hover:text-blue-900" title="Edit">

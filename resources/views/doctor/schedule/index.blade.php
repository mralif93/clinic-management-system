...
@section('content')
    <div class="space-y-6">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-emerald-100 text-sm mt-1">View and manage your appointment schedule</p>
                    </div>
                </div>
                <a href="{{ route('doctor.schedule.settings') }}"
                    ...
                    <i class='hgi-stroke hgi-settings-01 mr-2'></i> Schedule Settings
                </a>
            </div>
        </div>
        ...
        <!-- Date Selector -->
        <div
            class="bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 rounded-2xl border border-emerald-100/50 p-5">
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex-shrink-0 text-center w-16">
                                        <div class="text-2xl font-bold {{ $sConfig['time_text'] }}">
                                            {{ \Carbon\Carbon::parse($appointment->appointment_time)->format('h:i') }}</div>
                                        <div class="text-xs font-semibold {{ $sConfig['time_text'] }} opacity-70 uppercase">
                                            {{ \Carbon\Carbon::parse($appointment->appointment_time)->format('A') }}</div>
                                    </div>
                                    ...
                                    <div class="flex-shrink-0">
                                        <div
                                            class="w-11 h-11 bg-white rounded-full flex items-center justify-center shadow-sm border border-gray-200">
                                            <span
                                                class="text-sm font-bold text-gray-600">{{ strtoupper(substr($appointment->patient->first_name ?? 'P', 0, 1)) }}{{ strtoupper(substr($appointment->patient->last_name ?? '', 0, 1)) }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-grow min-w-0">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 mt-1 text-sm text-gray-500">
                                            ...
                                            <span
                                                class="inline-flex items-center gap-1 text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-md font-medium">
                                            ...
                                            @if($appointment->fee)
                                                ...
                                                <span class="inline-flex items-center gap-1 text-xs">
                                                    <i class='hgi-stroke hgi-money-bag-01 text-green-500'></i>
                                                    <span
                                                        class="font-medium text-gray-600">RM{{ number_format($appointment->fee, 2) }}</span>
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-shrink-0 flex items-center gap-3">
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold {{ $sConfig['badge_bg'] }} {{ $sConfig['text'] }}">
                                            <i class='hgi-stroke {{ $sConfig['icon'] }}'></i>
                                            {{ ucfirst(str_replace('_', ' ', $appointment->status)) }}
                                        </span>
                                        <a href="{{ route('doctor.appointments.show', $appointment->id) }}"
                                            class="w-9 h-9 flex items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-500 text-white rounded-full hover:from-emerald-600 hover:to-teal-600 transition-all shadow-sm hover:shadow-md group-hover:scale-105"
                                            title="View Details">
                    ...
                    @for($date = $weekStart->copy(); $date <= $weekEnd; $date->addDay())
                        ...
                        <a href="{{ route('doctor.schedule.index', ['date' => $date->format('Y-m-d')]) }}"
                            ...
                            <div class="text-center mb-2">
                                <div
                                    class="text-xs font-semibold {{ $isToday ? 'text-emerald-600' : 'text-gray-500' }} uppercase">
                                    {{ $date->format('D') }}</div>
                                <div class="text-xl font-bold {{ $isToday ? 'text-emerald-700' : 'text-gray-900' }}">
                                    {{ $date->format('d') }}</div>
                            </div>
                            <div class="space-y-1">

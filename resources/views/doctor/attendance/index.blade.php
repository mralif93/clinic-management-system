...
@section('content')
    <div class="space-y-6">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-emerald-100 text-sm mt-1">Track your attendance and work hours</p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="text-center p-4 bg-gray-50 rounded-xl">
                            ...
                            @if($todayAttendance->status === 'present')
                                <span
                                    class="px-3 py-1.5 rounded-lg text-sm font-semibold bg-emerald-100 text-emerald-700">Present</span>
                            ...
                            @else
                                <span
                                    class="px-3 py-1.5 rounded-lg text-sm font-semibold bg-gray-100 text-gray-700">{{ ucfirst(str_replace('_', ' ', $todayAttendance->status)) }}</span>
                            @endif
                        </div>
                        <div class="text-center p-4 bg-emerald-50 rounded-xl">
                            <p class="text-xs font-semibold uppercase tracking-wider text-emerald-600 mb-2">Clock In</p>
                            <p class="text-xl font-bold text-emerald-700">{{ $todayAttendance->clock_in_time->format('h:i A') }}
                            </p>
                        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div
                        class="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-white shadow-lg shadow-amber-500/30 group-hover:scale-110 transition-transform">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                <h3 class="text-base font-semibold text-gray-900 flex items-center gap-2">
                    <i class='hgi-stroke hgi-time-schedule text-blue-500'></i> Attendance History
                </h3>
            </div>
                        ...
                        @forelse($attendances as $attendance)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $attendance->date->format('M d, Y') }}
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-lg bg-emerald-100 flex items-center justify-center">
                                            <i class='hgi-stroke hgi-login-01 text-emerald-600 text-sm'></i>
                                        </div>
                                        <span
                                            class="text-sm font-medium text-gray-900">{{ $attendance->clock_in_time->format('h:i A') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($attendance->clock_out_time)
                                        <div class="flex items-center gap-2">
                                            <div class="w-7 h-7 rounded-lg bg-red-100 flex items-center justify-center">
                                                <i class='hgi-stroke hgi-logout-01 text-red-600 text-sm'></i>
                                            </div>
                                            <span
                                                class="text-sm font-medium text-gray-900">{{ $attendance->clock_out_time->format('h:i A') }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($attendance->total_hours)
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 bg-purple-100 text-purple-700 rounded-lg text-sm font-semibold">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($attendance->status === 'present')
                                        <span
                                            class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700">Present</span>
                                    @elseif($attendance->status === 'late')
                                        <span
                                            class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-100 text-amber-700">Late</span>
                                    @elseif($attendance->status === 'half_day')
                                        <span class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-100 text-blue-700">Half
                                            Day</span>
                                    @elseif($attendance->status === 'absent')
                                        <span
                                            class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-100 text-red-700">Absent</span>
                                    @else
                                        <span
                                            class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-gray-100 text-gray-700">{{ ucfirst(str_replace('_', ' ', $attendance->status)) }}</span>
                                    @endif
                                </td>
                            </tr>
                        @empty

...
@section('content')
    <div class="space-y-6">
        <!-- Page Header -->
        <div class="bg-gradient-to-r from-orange-600 to-amber-600 rounded-2xl p-6 text-white  mb-6 shadow-lg relative overflow-hidden">
    ...
    <div>
        ...
        <p class="text-orange-100 text-sm mt-1">Select a month to view attendance records</p>
    </div>
</div>
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.attendance.live') }}"
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-white/20 backdrop-blur text-white rounded-xl font-medium hover:bg-white/30 transition-all">
                        <i class='hgi-stroke hgi-record text-lg'></i>
                        Live Dashboard
                    </a>
                        ...
                        <div
                            ...
                            <div class="p-6">
                                <div class="flex items-center justify-between mb-4">
                                    <div
                                        class="bg-green-100 p-3 rounded-full group-hover:bg-green-600 transition-colors duration-300">
                                        <i
                                            class='hgi-stroke hgi-calendar-03 text-2xl text-green-600 group-hover:text-white transition-colors duration-300'></i>
                                    </div>
                                    @if($isCurrentMonth)
                                        <span
                                ...
                                <div class="flex items-center justify-between text-sm border-t border-gray-100 pt-4">
                                    <div class="flex gap-2 w-full justify-between">
                                        <span
                                            ...
                                            title="Present">
                                            <i class='hgi-stroke hgi-checkmark-circle-02'></i> {{ $month->present_count }}
                                        </span>
                                        <span
                                            class="flex items-center gap-1 text-xs text-yellow-600 bg-yellow-100 px-2 py-0.5 rounded-full"
                                            title="Late">
                                            <i class='hgi-stroke hgi-clock-01'></i> {{ $month->late_count }}
                                        </span>
                                        <span
                                            class="flex items-center gap-1 text-xs text-red-600 bg-red-100 px-2 py-0.5 rounded-full"
                                            title="Absent">
                                            <i class='hgi-stroke hgi-cancel-circle text-xs'></i> {{ $month->absent_count }}
                                        </span>
                                    </div>
                                </div>
                            </div>

...
@section('content')
    <div class="w-full">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900">Create New To-Do</h2>
                <p class="text-sm text-gray-600 mt-1">Add a new task to your to-do list</p>
            </div>
            ...
            <form action="{{ route('admin.todos.store') }}" method="POST">
                    ...
                    <div>
                        ...
                        <select name="priority" id="priority" required
                            ...
                            @foreach(\App\Models\Todo::getPriorities() as $key => $label)
                                <option value="{{ $key }}" {{ old('priority', 'medium') == $key ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        ...
                        <i class='bx bx-save mr-2'></i>Create To-Do
                    </button>
                </div>
            </form>
        </div>

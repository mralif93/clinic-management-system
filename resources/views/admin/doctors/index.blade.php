    ...
    <div class="space-y-6">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    <div class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                        <i class='hgi-stroke hgi-stethoscope text-2xl'></i>
                    </div>
                    <div>
                        <h2 class="text-2xl font-bold">Doctor Management</h2>
                        <p class="text-green-100 text-sm mt-1">Manage doctor profiles and information</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    ...
                    <p class="text-sm text-gray-500 font-medium mt-1">Total Doctors</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <p class="text-2xl font-bold text-gray-900">{{ $doctors->where('is_available', true)->count() }}</p>
                    <p class="text-sm text-gray-500 font-medium mt-1">Available</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <p class="text-2xl font-bold text-gray-900">{{ $doctors->where('type', 'psychology')->count() }}</p>
                    <p class="text-sm text-gray-500 font-medium mt-1">Psychology</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <p class="text-2xl font-bold text-gray-900">{{ $doctors->where('type', 'general')->count() }}</p>
                    <p class="text-sm text-gray-500 font-medium mt-1">General</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-3 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
                    ...
                    <p class="text-xs text-gray-500">
                        Showing {{ $doctors->firstItem() ?? 0 }} &ndash; {{ $doctors->lastItem() ?? 0 }} of {{ $doctors->total() }} results
                    </p>
                </div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @elseif($doctor->is_available)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-green-50 text-green-700 ring-1 ring-inset ring-green-500/20">
                                        <i class='hgi-stroke hgi-checkmark-circle-02'></i> Available
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-500/20">
                                        <i class='hgi-stroke hgi-clock-02'></i> Unavailable
                                    </span>
                                @endif
                            </td>
                            ...
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    <div class="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center mb-4">
                                        <i class='hgi-stroke hgi-plus-sign text-4xl text-gray-400'></i>
                                    </div>

...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600 mt-1">Manage doctor profiles and information</p>
        </div>
        <a href="{{ route('admin.doctors.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition flex items-center">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" action="{{ route('admin.doctors.index') }}" class="flex flex-wrap gap-4">
            ...
            @if(request()->hasAny(['search', 'type', 'status']))
                <a href="{{ route('admin.doctors.index') }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400 transition">
                    Clear
                </a>
            @endif
        </form>
                    ...
                    @forelse($doctors as $doctor)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-green-600">{{ $doctor->doctor_id ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-full bg-green-100 flex items-center justify-center">
                                            <span class="text-green-600 font-semibold">{{ strtoupper(substr($doctor->first_name, 0, 1)) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($doctor->trashed())
                                        ...
                                        <form action="{{ route('admin.doctors.force-delete', $doctor->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to permanently delete this doctor? This action cannot be undone!');">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        <a href="{{ route('admin.doctors.show', $doctor->id) }}" class="text-blue-600 hover:text-blue-900" title="View">
                                            <i class='bx bx-show text-xl'></i>
                                        </a>
                                        <a href="{{ route('admin.doctors.edit', $doctor->id) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                            <i class='bx bx-edit text-xl'></i>
                                        </a>
                                        <form action="{{ route('admin.doctors.destroy', $doctor->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this doctor?');">
                                            @csrf
                                            @method('DELETE')

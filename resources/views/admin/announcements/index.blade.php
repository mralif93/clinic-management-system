    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($announcements as $announcement)
                <div
                    class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all group {{ $announcement->trashed() ? 'opacity-60' : '' }}">
                    <!-- Image Preview -->
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    ...
                                    @if($announcement->is_featured)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-yellow-100 text-yellow-700">
                    ...
                    <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                        <div class="flex items-center gap-2">
                            ...
                            @elseif($announcement->is_published)
                                <span
                                    class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-green-50 text-green-700">Published</span>
                            @else
                        ...
                        <div class="flex gap-2">
                            @if($announcement->trashed())
                                ...
                                <form action="{{ route('admin.announcements.force-delete', $announcement->id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        onclick="return confirm('Permanently delete this announcement? This cannot be undone!')"
                            ...
                            @else
                                ...
                                <form action="{{ route('admin.announcements.toggle-publish', $announcement->id) }}" method="POST"
                                    ...
                                    <button type="submit"
                                        ...
                                        <i
                                            class='hgi-stroke {{ $announcement->is_published ? 'hgi-view-off-slash' : 'hgi-eye' }} text-lg'></i>
                                    </button>
                                </form>
                                <form action="{{ route('admin.announcements.toggle-featured', $announcement->id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    <button type="submit"
                                        ...
                                        <i
                                            class='hgi-stroke {{ $announcement->is_featured ? 'hgi-star' : 'hgi-star' }} text-lg'></i>
                                    </button>
                                </form>

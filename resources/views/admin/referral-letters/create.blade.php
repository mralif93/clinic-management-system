...
<div class="space-y-6 max-w-4xl mx-auto">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                <div class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                    <i class='hgi-stroke hgi-file-attachment text-2xl'></i>
                </div>
                <div>
                    <h2 class="text-2xl font-bold">New Referral Letter</h2>
                    <p class="text-blue-100 text-sm mt-1">Create a formal referral letter as an administrator</p>
                </div>
            </div>
    ...
    <form action="{{ route('admin.referral-letters.store') }}" method="POST" class="space-y-4">
        @csrf
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <textarea name="reason" rows="5" required placeholder="Provide a clear clinical reason for the referral…"
                        class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('reason') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="clinical_notes" rows="4" placeholder="Relevant history, investigations, medications…"
                        class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('clinical_notes') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit"
                class="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-500/30">

    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-5 border-b border-gray-100 bg-gray-50/50">
            <div class="flex items-center gap-2">
                <i class='bx bx-toggle-left text-purple-600'></i>
                <h3 class="font-semibold text-gray-700">Module Visibility & Order</h3>
            </div>
            <p class="text-sm text-gray-500 mt-1">Control whether the Packages module appears on the public website</p>
        </div>
        <div class="p-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="border border-gray-200 rounded-xl p-4 {{ $modulePage->is_published ? 'bg-green-50/50 border-green-200' : 'bg-gray-50/50' }}">
                    ...
                    <form action="{{ route('admin.packages.toggle-visibility') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" 
                ...
                <div class="border border-gray-200 rounded-xl p-4 bg-gray-50/50">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-lg bg-purple-100 flex items-center justify-center">
                            <i class='bx bx-sort text-xl text-purple-600'></i>
                        </div>
                        <div>
                    ...
                    <form action="{{ route('admin.packages.update-order') }}" method="POST" class="flex gap-2">
                        @csrf
                        <input type="number" name="order" value="{{ $modulePage->order }}" min="0" 
                            class="flex-1 px-3 py-2 rounded-lg border border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all text-sm">
                        <button type="submit" 

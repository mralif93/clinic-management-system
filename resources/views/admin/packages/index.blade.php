    ...
    <div class="space-y-6">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-purple-100 text-sm mt-1">Manage special packages and offers</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                ...
                <p class="text-sm text-gray-500 font-medium mt-1">Total Packages</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                <p class="text-2xl font-bold text-gray-900">{{ $deletedPackages }}</p>
                <p class="text-sm text-gray-500 font-medium mt-1">Deleted</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                <p class="text-2xl font-bold text-gray-900">{{ $totalPackages - $activePackages }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($packages as $package)
                <div
                    class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all group {{ $package->trashed() ? 'opacity-60' : '' }}">
                    <!-- Card Header -->
                    <div class="p-5 border-b border-gray-100">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                                    <i class='hgi-stroke hgi-package text-2xl text-white'></i>
                                </div>
                                <div>
                            ...
                            @else
                                <span
                                    class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-700">Inactive</span>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="bg-gray-50 rounded-xl p-3">
                            @if($package->original_price)
                                <div class="flex items-baseline gap-2 mb-1">
                                    <span
                                        class="text-sm text-gray-400 line-through">RM{{ number_format($package->original_price, 0) }}</span>
                                </div>
                            @endif
                            <p class="text-2xl font-bold text-gray-900 text-purple-600">
                                RM{{ number_format($package->price, 0) }}</p>
                        </div>
                    </div>
                    ...
                    <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-end gap-2">
                        @if($package->trashed())
                            ...
                            <form action="{{ route('admin.packages.force-delete', $package->id) }}" method="POST" class="inline">
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    onclick="return confirm('Permanently delete this package? This cannot be undone!')"
                ...
                <div class="col-span-full">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
                        <div class="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4">
                            <i class='hgi-stroke hgi-package text-4xl text-gray-400'></i>
                        </div>

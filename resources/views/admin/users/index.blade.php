...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    <div class="shrink-0 w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center shadow-inner border border-white/20 transform transition-transform hover:scale-105">
                        <i class='hgi-stroke hgi-user-account text-2xl'></i>
                    </div>
                    <div>
                        <h2 class="text-2xl font-bold">User Management</h2>
                        <p class="text-violet-100 text-sm mt-1">Manage system users, roles, and permissions</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    ...
                    <p class="text-sm text-gray-500 font-medium mt-1">Admins</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <p class="text-2xl font-bold text-gray-900">{{ $doctorCount }}</p>
                    <p class="text-sm text-gray-500 font-medium mt-1">Doctors</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
                    <p class="text-2xl font-bold text-gray-900">{{ $patientCount }}</p>
                    <p class="text-sm text-gray-500 font-medium mt-1">Patients</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-3 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
                    ...
                    <p class="text-xs text-gray-500">
                        Showing {{ $users->firstItem() ?? 0 }} &ndash; {{ $users->lastItem() ?? 0 }} of {{ $users->total() }} results
                    </p>
                </div>
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors {{ $user->trashed() ? 'bg-red-50/30' : '' }}">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1 text-xs text-amber-600 mt-1">
                                        <i class='hgi-stroke hgi-clock-02'></i> Unverified
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ring-1 ring-inset {{ $roleStyles[$user->role] ?? $roleStyles['patient'] }}">
                                    <i class='hgi-stroke {{ $roleIcons[$user->role] ?? 'hgi-user' }}'></i>
                                    {{ ucfirst($user->role) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @elseif($user->failed_login_attempts > 0)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-700">
                                        <i class='hgi-stroke hgi-alert-02'></i> {{ $user->failed_login_attempts }}/5
                                    </span>
                                @else
        ...
        Swal.fire({
            ...
            html: `<div class="text-left">
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-3">
                    <p class="text-sm text-red-700"><i class='hgi-stroke hgi-alert-circle mr-1'></i><strong>Warning:</strong> This cannot be undone!</p>
                </div>
            </div>`,
            icon: 'error',
            showCancelButton: true,
            confirmButtonColor: '#dc2626',
            cancelButtonColor: '#6b7280',
            confirmButtonText: '<i class="hgi-stroke hgi-cancel-circle mr-1"></i> Delete Permanently',
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.fire({ title: 'Deleting...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
